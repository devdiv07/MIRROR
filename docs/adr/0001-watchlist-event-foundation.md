...
This replaces the single "first implementation PR". Each PR is merged only when CI is green. Each keeps the existing 31 tests passing, adds no packages, makes no network calls in tests, and changes nothing in `src/scoring`, `src/enrichment`, `src/parsers/insider_parser_v2.py` or `legacy/`. Every acceptance case in §10 is a named test in the PR shown in its column.
