...
- **Retries.** At most 3 attempts per HTTP request, with exponential backoff (1 s, 2 s, 4 s) on 429/5xx/timeout. No retry on 4xx other than 429. The SEC adapter keeps a global cap below 10 requests/second (it reuses the existing 0.5 s sleep) and sends the declared User-Agent.
  - *As built (2026-09-24):* two waits (1 s, 2 s) for 3 attempts. One rate limit covers **every** SEC request of a run, including additional files and retries: at least 0.5 s between request starts. Invalid JSON on a 200 is retried like a 5xx. A response of the wrong shape (missing or unequal columns, non-text values, bad dates or accession numbers, a non-UTC `acceptanceDateTime`, an additional-file name outside `CIK##########-submissions-###.json`, another company's CIK) is not retried. Either way the request fails: a bad submissions JSON fails the listing's check, and a bad additional file leaves its period missing. `SEC_USER_AGENT` must be set, with a contact email. There is no built-in contact. Without it, `ingest` stops before any request and records nothing, because this is a configuration error, not a source failure.
...
- **SEC fetch coverage and backfill** (contract fixed 2026-09-23). The submissions JSON holds "at least one year's of filing or to 1,000 (whichever is more) of the most recent filings" in `filings.recent`. Older filings are in additional files listed in `filings.files` with their date ranges **[src]** [SEC EDGAR APIs](https://www.sec.gov/search-filings/edgar-application-programming-interfaces). An additional file uses the same columnar fields as `recent` **[run]** (Apple, CIK 320193: `recent` back to 2015-07-27; one file covering 1994-01-26 to 2015-07-25). A fetch at time `F` therefore **covers** the union of:
  ...
  **Amendment (2026-09-24): gaps, and the times of a slow fetch.**
  - *Gaps are kept, not skipped.* A `partial` check saves the spans it covered in `coverage_span` (schema v3). The next run without `--sec-since` starts at `db.resume_point`. Walking forward from the earliest window any SEC check was asked to cover (`ok`, `partial` or `failed`), this is the first moment that no `ok` window or `partial` span covers. A partial backfill, including a partial *first* run, or a failed run is therefore retried by every ordinary run until it is fetched. Until then, windows that overlap the gap stay `coverage_incomplete`. `coverage()` counts `partial` spans, so the days that were covered still read as checked. Previously the next run resumed from the end of the latest `ok`/`partial` check, which skipped the gap.
  - *Recent's lower bound.* Filings after the newest file's `filingTo` can only be in `recent`, so `recent` covers from 00:00 ET on the day after the **earlier** of its oldest `filingDate` and that `filingTo`. Found live: Apple's file ends 2015-07-25 and `recent` starts 2015-07-27, and the original rule left 07-26..27 uncovered for ever ([run note](../runs/2026-09-23-sec-live-check.md)).
  - *The oldest additional file reaches back to the beginning,* as `recent` does when there are no files: together, `recent` and the files are the company's whole EDGAR history. A backfill that starts before a company's first filing is then `ok`, not partial for ever.
  - *Cutoff vs. arrival.* `F` is taken before any request. Every window ends at `F`, and only filings with `published_at ≤ F` are stored. A filing accepted while a response is in flight is left for the next run, whose window starts at `F`. `first_seen_at` is when the response holding the filing **arrived**. `checked_at` and `ingest_run.finished_at` are when they were recorded. None of them is ever earlier than `F`. Previously all three were `F`, so a slow response made a filing look known before MIRROR had it.
...
**Milestone 2 amendment (2026-09-23).** `event` gains `version INTEGER NOT NULL` and `content_sha256 TEXT NOT NULL`. `UNIQUE (dedup_key)` becomes `UNIQUE (dedup_key, version)` (§4.2, "Event identity and versions"). `source_document` drops `UNIQUE (source, source_doc_key, content_sha256)`, which would forbid recording an A → B → A change. Schema version 2. Schema v1 had no code that wrote documents or events, so `init_schema` upgrades a v1 database by recreating those two tables when they are empty, and refuses (rather than guessing) if they are not.

**Schema v3 (2026-09-24).** Adds `coverage_span (check_id, covered_from, covered_to)`: the parts of a `partial` check's window that it covered (§4.2 amendment). An `ok` check covers its whole window. A `partial` check covers exactly its spans, and none means nothing is known to be covered, for example a manual check limited by a scope note. The change is additive, so `init_schema` upgrades a v2 database in place. Its `partial` checks have no spans, so their windows are retried.
