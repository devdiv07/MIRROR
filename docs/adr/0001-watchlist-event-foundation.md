...
- **[run]** GitHub Actions CI fails at its first step (`pyflakes`) on `main` at the baseline, so pytest does not run in CI (fixed by PR-0, §13).
...
- **Questions to answer, with primary evidence (written replies or published terms):**
  ...
  2. Which NSE Data & Analytics product delivers corporate announcements, results and actions? What are its format, latency, price and terms for an individual, non-redistributing subscriber?
...
- **Timebox:** the investigation concludes **before PR-4 merges** (§13). If a question is still open then, ADR 0002 records it as *unresolved*. India stays a prototype: its output keeps showing its computed coverage state (§4.4), and it is excluded from time-saving measurements.
- **Allowed outcomes:** automate via a permitted route (new adapter PR); keep manual entry and accept its labelled coverage; or drop India from the pilot. Scraping pages is not an allowed outcome while the current website terms stand.
...
| `published_at` | When the source made it public | SEC `acceptanceDateTime` **[run]**. Whether its `Z` suffix is true UTC is **[unv]**; PR-2 checks it against the EDGAR filing index page. | NSE dissemination time as entered by the user. RSS `pubDate` has no timezone **[run]**, so IST is an **[inf]** assumption and the event is flagged `tz_assumed=1` |
...
- Nothing with `first_seen_at > T` (live) or `published_at > T` (replay) may be presented as then-known. This is a test in PR-2 (§10, case R1).
...
Session open and close times come from config per exchange. The values themselves (NSE 09:15–15:30 IST, NYSE/Nasdaq 09:30–16:00 ET) are **[unv]** here. PR-1 cites the exchange source in the config file.
...
- **Pilot, in two parts.**
  - (a) **US utility pilot**: 10–20 stocks the owner follows, for at least 4 weeks **[inf]**, once PR-4 is merged. SEC coverage is then measured automatically.
...
| # | PR | Case | Fixture | Expected |
|---|---|---|---|---|
| **C1** | PR-2 | **Not checked** (India) | Watched NSE security; no `coverage_check` row for `MANUAL_NSE` in the window; no events | Coverage state `not_checked`. Listed under *Not checked*, **not** under *Checked, no new disclosures* |
| **C2** | PR-2 | **Checked, no events** (India) | Same security; `checked NSE <symbol> --through <window end>` recorded (`ok`, spans the window); no events | State `checked_no_events`. Listed under *Checked, no new disclosures* with "checked through <time>" |
| C3 | PR-2 | Coverage incomplete | Manual check through the middle of the window only, **or** a check with `--partial --note "results page only"` | State `coverage_incomplete`, showing the covered span or the note |
| C4 | PR-2 | Event entered, no check | One manual NSE event entered in the window; no `coverage_check` | The event is shown under "What changed", but NSE coverage stays `not_checked`. Entering an event does not imply a check |
| C5 | PR-2 | US checked, no events | SEC fixture fetch succeeds for the CIK; no new filings in the window | Automatic `ok` check. State `checked_no_events` |
| A1 | PR-4 | Verified pre-open result (India) | Manual NSE results event, `published_at` 08:40 IST on `D`; NSE check `ok` through `close(D)`; `+8%` move on `D`; trigger 5 | Card with `documented_event`, tag `pre_open`, verbatim subject, link; no coverage banner |
| A2 | PR-4 | Post-move news or filing | US 8-K with `acceptanceDateTime` after the close on `D`; SEC check `ok`; `+11%` move on `D` | Event listed under "Reported after the move". Label `no_verified_explanation_yet` |
| A3 | PR-3 | Split or bonus day | 1:1 bonus (`new_per_old = 2`) ex on `D`; raw close halves, adjusted move `+1%` | No card (below trigger), and no −50% anywhere. The same fixture without the action row raises `possible_unrecorded_corporate_action` and holds the card |
| A4 | PR-4 | Unexplained +10%, full coverage (US) | `+10%` move, benchmark `+0.3%`, no events, SEC check `ok` spanning the window | `no_verified_explanation_yet`; sources checked and their times listed; no causal wording; no banner |
| **A4b** | PR-4 | Unexplained +10%, **incomplete coverage** (India) | `+10%` move, no events, **no** NSE check for the window | `no_evidence_coverage_incomplete`, with the banner "⚠ COVERAGE INCOMPLETE: NSE not checked…" as the card's first line. Never `no_verified_explanation_yet` |
| A5 | PR-3 | Stale or missing price | Calendar expects `D`; last bar is `D-1` | Item marked `stale`, "price as of D-1", no move computed |
| A6 | PR-2 (state) / PR-4 (render) | Source outage | SEC adapter returns HTTP 503 ×3 | `ingest_run.status = failed`; `coverage_check.status = failed`; the security is listed under *Source failed*; the header shows "SEC EDGAR: failed at <time>, last success <time>"; the brief still renders |
| **P1** | PR-3 | Price file with unknown upstream | CSV with no vendor or URL declared | Card shows "Upstream price source: unknown (owner-supplied file …, sha256 …, imported …)" plus the calculation trace. The move is shown |
| P2 | PR-3 | Price number without provenance | A `price_bar` row whose `import_id` is missing or doesn't resolve (fixture forces it) | Rendering refuses the number: "price data missing provenance". The test asserts no bare number appears |
| R1 | PR-2 | As-of leakage guard | Event with `first_seen_at` = `D+1 09:00`; brief generated as of `D+1 08:00` | Event absent from that brief |
| R2 | PR-4 | Market-wide move | Stock `+6%`, benchmark `+5.5%`, results event pre-open, coverage complete | `multiple_factors` |
...
| Buy an NSE corporate-data subscription now | The product, price and individual eligibility are **[unv]** (§4.1). This is evaluated in §4.3 against the manual-entry cost measured in the prototype. |
...
## 13. Implementation plan: reviewable PRs

This replaces the single "first implementation PR". Each PR is merged only when CI is green. Each keeps the existing 31 tests passing, adds no packages, and makes no network calls in tests. PR-1 to PR-4 change nothing in `src/scoring`, `src/enrichment`, `src/parsers/insider_parser_v2.py` or `legacy/`. PR-0 touches existing files for lint only, with no behaviour change. Every acceptance case in §10 is a named test in the PR shown in its column.

**PR-0 — CI repair (lint only).** CI has been red on `main` since `e068c91`: `pyflakes src/ tests/` fails on existing code (unused imports, f-strings without placeholders), so the pytest and pip-audit steps have never run in CI **[run: GitHub Actions runs 27905872952 and 35853057284]**.
- *Change:* remove the flagged unused imports and placeholder-free `f` prefixes, plus one unused local variable (`this_insider`, [cluster.py:71](../../src/scoring/cluster.py)). Removing that variable does not change any score. Some imports exist only to trigger `@register_scorer`, in `src/scoring/__init__.py` and `tests/test_entity_filer.py`. **Keep them; deleting them changes behaviour.** Pyflakes itself ignores `# noqa`, but it does treat names listed in `__all__` as used **[run: pyflakes 4.0.0 in a scratch venv]**. So list them in `__all__` in the package `__init__`, and reference the modules explicitly in the test. `pyflakes src/ tests/` reports 44 findings at the baseline **[run]**.
- *Must not:* remove or skip the pyflakes step, or change any scoring output.
- *Pass:* CI green on the PR, **including the pytest and pip-audit steps actually running**, with their results reported in the PR. If `pip-audit --strict` then fails, report it and fix it in the same PR or a follow-up; do not disable it.

**PR-1 — Store, identity, calendar config.**
...
**PR-2 — Event foundation and coverage.**
...
**PR-3 — Prices, corporate actions, moves.**
...
**PR-4 — Timing, evidence labels, brief, feedback.**
...
- *Pass:* the cases and golden test pass. One manual run on the owner's real watchlist renders a brief. The PR description reports what was observed: counts per coverage group, any provenance failures, and the upstream price source shown. This demonstrates that the pipeline works on real data. **It is not evidence of time saved.**

**After PR-4:** the US utility pilot can start (§9). India stays a prototype until ADR 0002 (§4.3), which must be concluded, even if as "unresolved", before PR-4 merges.

## 14. Open questions

| # | Question | Blocks | Resolved in |
|---|---|---|---|
| Q1 | Which India disclosure route is permitted for MIRROR's use: NSE RSS polling (needs written confirmation), an NSE Data & Analytics corporate-data subscription, BSE, a broker API, or none? What does each cost? | Any India time-saving pilot; India automation | §4.3 → ADR 0002, before PR-4 merges |
...
| Q3 | Is SEC `acceptanceDateTime` with `Z` actually UTC? | Exact timing tags for US events near session boundaries | PR-2 |
| Q4 | Exchange session times and holiday lists: which primary sources? | `calendar.py` config | PR-1 |
| Q5 | India macro sources (RBI, MOSPI) and their terms | Macro in slice 2 | Later |
| Q6 | Which issuer identifier to use for India (ISIN vs exchange symbol vs company registration number)? | Robust symbol-change handling in India | PR-1 (provisional), ADR 0002 |
