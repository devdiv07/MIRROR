...
- **Result:** contracts fixed first in `7cf134d` (fifth state `checked_with_events`; event versions with an as-of rule; SEC backfill coverage). `e2e2771` rejects NaN/infinite move triggers. `1917d63` is schema v2 (versioned events/documents). `9493792` adds the SEC adapter, manual NSE events, coverage states and the CLI. `bce14c2` updates the docs. CI on `bce14c2` ([run 35863687190](https://github.com/devdiv07/MIRROR/actions/runs/35863687190)): pyflakes clean · pytest **122 passed** · pip-audit clean. All tests are offline.
- **Hardening (2026-09-24), after review of `d8fa752`:** `09b12c7` requires `SEC_USER_AGENT` (no built-in contact) and applies one rate limit to every SEC request, including backfill files and retries. `f2213f8` records invalid JSON and wrong response shapes as fetch failures. `2258070` stamps `first_seen_at`/`checked_at` when responses arrive, while windows still end at the pre-fetch cutoff. `440c2c0` (schema v3, `coverage_span`) keeps gaps, so an ordinary run retries a partial or failed period, including a partial first run, until it is fetched. `91eac7d` fixes a coverage-boundary defect found live. CI on `440c2c0` ([run 35904227551](https://github.com/devdiv07/MIRROR/actions/runs/35904227551)): pyflakes clean · pytest **158 passed** · pip-audit clean.
- **Live check (2026-09-23 UTC, AAPL, temporary watchlist):** [runs/2026-09-23-sec-live-check.md](runs/2026-09-23-sec-live-check.md). 12/12 filing links returned 200. 3/3 acceptance times matched EDGAR's index pages. A repeat backfill added 0 versions. A 2010 backfill (1,188 events) exposed the boundary defect, and after `91eac7d` an ordinary rerun closed the gap. The owner's own watchlist has not been ingested yet.

### Step 4 — prices: prices, corporate actions, moves _(done 2026-09-24)_
- [x] Price CSV import with `price_import` provenance, corporate actions, `src/compute/moves.py`
...
- **Result:** `04b1f91` (price CSV import: file hash, declared upstream or `unknown`, newest import visible as of T wins), `13e2a7a` (corporate actions, adjusted moves with statuses, guard, volume ratio and price-claim lines), `3722100` (`ingest` imports `data/prices/*.csv` and `data/corporate_actions.csv`; new `moves` command). CI on `3722100` ([run 35905933630](https://github.com/devdiv07/MIRROR/actions/runs/35905933630)): pyflakes clean · pytest **224 passed** · pip-audit clean. A3, A5, P1, P2 and the guard are named tests in `tests/test_moves.py` and `tests/test_price_provenance.py`, and every number is recomputed from stored rows. Choices beyond the ADR text are listed in its Milestone 3 amendment (§13). The benchmark-relative move moves to Step 5 (A4, R2 need it). All inputs are still synthetic: no real price file has been imported yet.

### Step 5 — brief: timing, evidence labels, brief, feedback _(ACTIVE)_
- [ ] `timing.py`, `evidence.py`, `render_markdown.py`, the benchmark-relative move (moved from Step 4), and the `compute`, `brief`, `feedback` and `thesis` CLI commands
