...
def aggregate(row: dict, config: Dict[str, Dict[str, Any]] = None) -> float:
    ...
    """
    # Data-quality gate: entity filers (ESPP trusts, 401k plans, index funds)
    # are non-discretionary and carry zero information about insider sentiment.
    # The parser tags them with is_company=True; we hard-zero them here rather
    # than in a scorer so their removal is explicit and unconditional.
    if row.get('is_company', False):
        return 0.0
...
def explain(row: dict, config: Dict[str, Dict[str, Any]] = None) -> dict:
    ...
    """
    # Mirror the entity-filer gate from aggregate()
    if row.get('is_company', False):
        return {'contributions': {}, 'final_signal': 0.0, 'gated': 'entity_filer'}
